...
contract C is Base {
	function test() public view returns (uint r) {
        ...
        bool success;
        bytes memory result;
        (success, result) = t.staticcall(abi.encodeCall(X.a, 1));
        require(success && result.length == 32);
        r += abi.decode(result, (uint));
        require(r == 2);

        (success, result) = t.staticcall(abi.encodeCall(X.b, 10));
        require(success && result.length == 32);
        r += abi.decode(result, (uint));
        require(r == 13);

        (success, result) = t.staticcall(abi.encodeCall(Base.a, 100));
        require(success && result.length == 32);
        r += abi.decode(result, (uint));
        require(r == 114);

        (success, result) = t.staticcall(abi.encodeCall(this.a, 1000));
        require(success && result.length == 32);
        r += abi.decode(result, (uint));
        require(r == 1115);
